...
Double-click the file, or open it in a browser directly — `file:///…/xrd-studio.html` works
with no server. To host it (GitHub Pages, any static host), just serve that one file; there's
no build step, no `base` path to configure, no dependencies to install.
